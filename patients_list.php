    ...
    if (isset($_POST['filter'])) {
        $filter_date = $_POST['filter_date']; // Assuming it's in "YYYY-MM-DD" format
        $sql = "SELECT * FROM tbl_patient WHERE DATE(created_at) = '$filter_date'";
        $result = $conn->query($sql);
        $patients = [];
                            ...
                            <div class="col">
                                ...
                                <div>
                                    <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
                                        <?php $currentDate = date("Y-m-d"); // Example format: 2023-10-29 
                                        ?>
                                        <label for="filter">filter By Date</label>
                                        <input type="date" name="filter_date" value="<?php echo $currentDate;  ?>">
